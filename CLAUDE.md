...
**`AchievementBarChart` / SMKP category breakdown responsive layout**: both render a label + horizontal bar + value per row using fixed-width Tailwind classes (`w-48`, `w-28`, etc.) for the label/value columns so the bar's `flex-1` track lines up across rows — those fixed widths were sized for desktop only. On a ~390px mobile viewport the label+value alone exceed the available row width, so the `flex-1` bar shrinks to 0 and silently disappears (no console error, no layout break visible in a quick look — only obvious by actually measuring the rendered bar's width, e.g. via `getBoundingClientRect()`). The fix is a `flex-col sm:flex-row` row: mobile stacks label+value on one line and the full-width bar on the next; `sm:` restores the original fixed-width single-row layout unchanged. Any new label+bar+value row follows this same two-breakpoint pattern rather than a single fixed-width flex row.
